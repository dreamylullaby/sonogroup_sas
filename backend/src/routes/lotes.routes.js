...
router.get('/', async (req, res) => {
    try {
        const { limit = 50, offset = 0, topografia } = req.query;
...
// Actualizar características de un lote (requiere autenticación)
router.put('/:id', verificarToken, async (req, res) => {
    try {
        ...
        if (!inmueble) return res.status(404).json({ error: 'Lote no encontrado' });
        if (inmueble.id_usuario !== req.usuario.id_usuario && req.usuario.rol !== 'admin') {
            return res.status(403).json({ error: 'No tienes permisos para modificar este lote' });
        }
