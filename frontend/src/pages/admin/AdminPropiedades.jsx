...
import { Building2, Plus, Eye, Edit2, EyeOff, Trash2, Search, FileText, Clock, CheckCircle, AlertTriangle } from 'lucide-react'
...
export default function AdminPropiedades() {
  ...
  const handleEliminar = async (id) => {
    try {
      ...
      setSuccessModal({ type: 'delete', message: 'Propiedad eliminada exitosamente' })
      fetchData()
    } catch (e) {
      setConfirmDeleteProp(null)
  ...
  const handleEditSaved = () => {
    setEditModal(null)
    setSuccessModal({ type: 'edit', message: 'Propiedad editada con éxito' })
    fetchData()
  }
  ...
  return (
    <div>
      ...
      {/* Edit Modal */}
      {editModal && (
        <PropertyEditModal property={editModal} onClose={() => setEditModal(null)} onSaved={handleEditSaved} />
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,27,46,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '380px', border: '0.5px solid #e0d8ec' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <h3 style={{ fontSize: '16px', fontWeight: 500, color: '#241929', margin: '0 0 8px' }}>¿Eliminar propiedad #{confirmDeleteProp.id_inmueble}?</h3>
              <p style={{ fontSize: '13px', color: '#5A4864', lineHeight: 1.6, margin: 0 }}>
                Esta acción eliminará permanentemente la propiedad
                {confirmDeleteProp.tipo_inmueble ? ` (${confirmDeleteProp.tipo_inmueble})` : ''} en {confirmDeleteProp.ubicaciones?.municipio || 'ubicación desconocida'}.
                No se puede deshacer.
              </p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '20px' }}>
              <button
                onClick={() => handleEliminar(confirmDeleteProp.id_inmueble)}
                style={{ width: '100%', background: '#CC1E2B', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '12px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em', cursor: 'pointer' }}
              >
                <Trash2 size={12} style={{ verticalAlign: 'middle', marginRight: '4px' }} /> Eliminar permanentemente
              </button>
      ...
      {successModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,27,46,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '360px', border: '0.5px solid #e0d8ec' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#E6F9EE', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
            ...
            <div style={{ marginTop: '20px' }}>
              <button
                ...
                style={{ width: '100%', background: '#0A0F2C', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '12px', fontWeight: 500, cursor: 'pointer' }}
              >
                Aceptar
              </button>
            </div>
